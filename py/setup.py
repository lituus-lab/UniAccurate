...
NIMBLE_FILE = "UniAccurate.nimble"
...
def nim_project_dir():
    """Where UniAccurate.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building uniaccurate from "
...
# The sdist ships the pre-transpiled uniaccurate/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("uniaccurate", "_core.pyx")
ext = Extension(
    "uniaccurate._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("uniaccurate", "_core.c")],
